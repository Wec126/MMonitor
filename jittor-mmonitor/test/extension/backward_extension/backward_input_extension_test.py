...
def test_model():
    ...
    optimizer.step(loss)
    print(f"The shape of the median value obtained by the current extension is{model.input_grad.shape}") # 与输入维度相同
def test_linear():
    model = Model()
    backward_input_extension = BackwardInputExtension()
    (x,y) = prepare_data()
    ...
    optimizer.set_input_into_param_group((x,y))
    optimizer.step(loss)
    print(f"The shape of median value obtained by the current extension is{l.input_grad.shape}") # 与输入维度相同
def test_Conv2d():
    model = Model()
    backward_input_extension = BackwardInputExtension()
    (x,y) = prepare_data()
    ...
    optimizer.set_input_into_param_group((x,y))
    optimizer.step(loss)
    print(f"The shape of median value obtained by the current extension is{conv2d.input_grad.shape}") # 与输入维度相同
test_model()
test_linear()
test_Conv2d()
