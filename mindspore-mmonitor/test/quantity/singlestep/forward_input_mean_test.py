...
def test_linear():
    ...
    optimizer(grads)
    quantity.track(i)
    print(if_similar(quantity.get_output()[0], ops.mean(x_linear)))
    handle.remove()
def test_conv():
    ...
    optimizer(grads)
    quantity.track(i)
    print(if_similar(quantity.get_output()[0], ops.mean(x_conv)))
    handle.remove()
def test_default():
    ...
    for epoch in range(1):  # 单轮训练示例
        ...
        print(if_similar(quantity.get_output()[0], np.mean(x_default.asnumpy())))
    handle.remove()
