...
def test_linear():
    ...
    weight_norm_direct = ops.norm(ops.flatten(l.weight)).asnumpy().item()
...
def test_conv():
    ...
    weight_norm_direct = ops.norm(ops.flatten(cov.weight)).asnumpy().item()
    print(if_similar(quantity.get_output()[0], weight_norm_direct))
...
def test_default():
    ...
    weight_norm_direct = ops.norm(ops.flatten(bn.gamma)).asnumpy().item()
    print(if_similar(quantity.get_output()[0], weight_norm_direct))
